...
ARG QUARTUS_TAR_FILE="Quartus-20.1.1.720-linux-complete.tar"
...
RUN DEBIAN_FRONTEND=noninteractive apt-get update && apt-get install -y \
  ...
  texinfo zlib1g-dev gcc-multilib libncurses5 libncurses5-dev \
  \
  && ldconfig
...
RUN chmod og+x /opt && mkdir -p /opt/quartus && chown :quartus /opt/quartus && chmod g+rwx /opt/quartus
...
RUN echo "Downloading and extracting ${QUARTUS_TAR_FILE} from ${QUARTUS_TAR_HOST}" && \
  mkdir /home/quartus/installer && cd /home/quartus/installer && \
  wget -O- ${QUARTUS_TAR_HOST}/${QUARTUS_TAR_FILE} -q | \
  tar -xvf -

RUN cd /home/quartus/installer/components && ./QuartusSetup-20.1.1.720-linux.run --mode unattended --accept_eula 1 --installdir /opt/quartus
...
RUN echo "alias quartus='LD_PRELOAD=/lib/x86_64-linux-gnu/libudev.so.1 /opt/quartus/quartus/bin/quartus --64bit'" >>~/.bashrc
